...
export function TableHeader({ children, className = '', ...props }: HTMLAttributes<HTMLTableSectionElement>) {
  return (
    <thead className={`bg-slate-50 [&_tr]:border-b ${className}`} {...props}>
      {children}
    </thead>
  );
}
...
export function TableRow({ children, className = '', ...props }: HTMLAttributes<HTMLTableRowElement>) {
  return (
    <tr 
      className={`border-b border-slate-200 transition-colors hover:bg-slate-50 data-[state=selected]:bg-slate-50 ${className}`} 
      {...props}
    >
      {children}
    </tr>
...
export function TableHead({ children, className = '', ...props }: ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th 
      className={`h-11 px-4 text-left align-middle text-xs font-bold uppercase text-slate-600 [&:has([role=checkbox])]:pr-0 ${className}`} 
      {...props}
    >
      {children}
    </th>
